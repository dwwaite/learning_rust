...
impl SeqRecord {
    ...
    fn extend_sequence(&mut self, new_sequence: &str) {
        // Must simpler implementation here.
        self.sequence += &new_sequence.to_uppercase();
    }
    ...
    fn produce_kmer_tally(&self, window_size: usize) -> HashMap<String, i64> {
        ...
        // Cast the sequence field to a Vec<char>, then slice it with windows.
        // I think my preference will always be a left-hand type declaration instead of turbofish.
        let kmer_vector: Vec<char> = self.sequence.chars().collect();

        for kmer_array in kmer_vector.windows(window_size) {
